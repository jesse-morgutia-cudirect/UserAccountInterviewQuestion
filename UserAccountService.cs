...
namespace Interviews.UserAccounts
{
    // ──────────────────────────────────────────────
    // Domain models
    // ──────────────────────────────────────────────
    ...
    ///
    /// Rules:
    ///  1. Email must be a valid format (contains '@' and a domain).
    ///  2. Password must be at least 8 characters.
    ///  3. Registering with an already-used email returns failure (no exception).
    ///  4. On successful registration, password is stored as a hash (never plain text).
    ///  5. A welcome email is sent on registration; if it fails, registration still succeeds.
    ///  6. Login fails if the account is locked (LockedUntilUtc is in the future).
    ///  7. Three consecutive failed login attempts lock the account for 15 minutes.
    ///  8. A successful login resets the failed-attempt counter.
    ///  9. Password reset sends an email with a new reset token; throws UserNotFoundException
